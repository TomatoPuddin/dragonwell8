#
# Copyright (c) 2011, 2019, 2023, Oracle and/or its affiliates. All rights reserved.
...
[
  if test "x$VS_ENV_CMD" = x; then
    ...
    # In VS 2017, VS 2019 and VS 2022, the default installation is in a subdir named after the edition.
  ...
  if test "x$MSVC_DLL" = x; then
    if test "x$VCINSTALLDIR" != x; then
      ...
      if test "$VS_VERSION" -lt 2017; then
        ...
        # Probe: Using well-known location from VS 2017, VS 2019 and VS 2022
        if test "x$OPENJDK_TARGET_CPU_BITS" = x64; then
